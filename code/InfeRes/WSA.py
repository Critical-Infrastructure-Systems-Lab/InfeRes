...
# # [5] ============================  DATA CLIPPING and CREATING FREQUENCY MAP
    ...
    for filename in filtered_files:
        try:
            ...
            ndwiRescaled = ((ndwi - np.nanmin(ndwi)) / (np.nanmax(ndwi) - np.nanmin(ndwi))) * 2 - 1
            ndwiRescaled[ndwiRescaled >= 0.4] = 1   # Sure water
            ndwiRescaled[ndwiRescaled <= -0.4] = -1   # Sure non-water
            ndwiRescaled[(np.logical_and(ndwiRescaled > -0.4, ndwiRescaled < 0))] = -0.25  #dry non-water
            ndwiRescaled[(np.logical_and(ndwiRescaled >= 0, ndwiRescaled < 0.4))] = 0.25   #wet non-water
            ...
            x = ndwiA.ravel()
            km = KMeans(n_clusters=4, random_state=0)  #random_state=0, n_init=10
