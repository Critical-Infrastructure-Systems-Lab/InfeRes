...
def preprocessing(res_name, max_wl, res_built_year, point, boundary):   
 ...
 # # [0] ============================= Generate image with common overlapping area 
    ...
    file_lists = [filesL9, filesL8, filesL7, filesL5, filesS2]
    filtered_files = []
...
# # [1] ============================= DEM clip based on given extent   
    ...
    L8 = gdal_array.LoadFile('MaxExtent.tif').astype(np.float32)
    L8[np.isnan(L8)==1]= np.nanmin(L8)  
    L8rescaled = ((L8 - np.nanmin(L8)) / (np.nanmax(L8) - np.nanmin(L8))) * 2 - 1
    aaL8=np.copy(L8rescaled).astype(np.float32)
    
    #-------------------- K-means clustering for maximum water extent -------------------- 
    x = aaL8.ravel()
    km = KMeans(n_clusters=2, random_state=0)
    km.fit(x.reshape(-1,1)) 
    cluster_labels = km.predict(aaL8.reshape(-1, 1))
    cluster_raster = cluster_labels.reshape(aaL8.shape)
    z = km.cluster_centers_
    max_center_index = np.argmax(np.sum(z, axis=1))
    max_cluster_mask1D = (cluster_labels == max_center_index)
    max_cluster_mask2D = max_cluster_mask1D.reshape(aaL8.shape)
    L8max_water_cluster = np.copy(max_cluster_mask2D).astype(np.float32)
    Max_Extent = np.copy(L8max_water_cluster).astype(np.float32)
    #-------------------- K-means clustering for maximum water extent --------------------
...
# # [2] ============================= Reservoir isolation  
    ...
    res_iso = pick(column, row, Max_Extent)
    res_isoN1 = expand(res_iso, 1)
...
# # [4] ===================  RESIZING all images and saving in a temporary folder (folder name is "ClipTemp") 
    ...
    for filename in filtered_files:
        try:           
           ...
           ndwiCloud[(np.logical_and(ndwiCloud <-0.4, ndwiCloud > -1))] = -3
           ...
           if int(filename[8:12]) < res_built_year:
              cloud_percentage = round(cloud_percentage-50,2)
           ...
           if cloud_percentage<=70:
               ...
               ndwiA[np.isnan(ndwiA) == 1] = -1
               ndwiA[res_areaN == 0] = -1
               ndwiA[ndwiA <-0.25] = -1    # assuming ndwi <-0.25 is a sure non-water pixel
               clipped_file = "Clipped_" + filename # Output file name
               output_image_path = os.path.join(output_folder, clipped_file)
               output = gdal_array.SaveArray(ndwiA.astype(gdal_array.numpy.float32), 
                                             output_image_path, 
                                             format="GTiff", prototype = dem_file_path)
               output = None
               print('-------------------------------------------------')
